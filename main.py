...
# 8271080c-47b9-4e73-b532-3aec88150bab дополнительный
listIn = []   # список в который будут записываться координаты
nameCoordJason = ""   # в каждом наборе данных наименование атрибута где можно найти координаты разное
...
def get_geoData(url, url1, url2, countItems):
    ...
    resp = requests.get(url=url, headers=headers)   # получение запроса с сайта
    current_page: int = 1

    while resp.status_code == 200:   # цикл перебора страниц и считывания данных с координатами
        url = url1 + str(current_page) + url2
        resp = requests.get(url=url, headers=headers)
        print(current_page, resp.status_code)
        current_page = current_page + 1
        jsonData = json.loads(resp.text)   # преобразование полученного текста с сайта в формат json
        global nameCoordJason

# поиск информации с координатами и подсчет записей
    ...
    df = pd.DataFrame(listIn)   # подготовка к записи данных в excel
    df.to_excel('./coordinates.xlsx')
    return countItems


# функция анализирует данные и ведет подсчет вхождений обьекта в опред категорию
...
# каждая запись координат отправляется в геокодер для получения метаданных
    for item in listIn:
        strCoord = str(item)
        print(strCoord)
        strCoord = strCoord.replace("[", "")   # оформление правильной записи координат для чтения геокодером
...
# вытаскиваем необходимый критерий из полученных данных ( принадлежность к россии/ к спб/ или к ло)
    ...
    countCoord = [count1, count2, count3]   # подсчет вхождений в критерии
    return countCoord
# парсинг страниц и запись данных в список с результатами
...
def parse_geoData():
    ...
    countItems = 0
    countCoord = [0, 0, 0]
    url = ('https://data.gov.spb.ru/api/public/version/5189/structure_version/568/?search_all='
           '%D1%84%D0%BE%D1%82%D0%BE%D1%80%D0%B0%D0%B4&page=1&number=&adress=&data_display='
           '&per_page=50')
    url1 = ('https://data.gov.spb.ru/api/public/version/5189/structure_version/568/?search_all='
           '%D1%84%D0%BE%D1%82%D0%BE%D1%80%D0%B0%D0%B4&page=')
    url2 = ('&number=&adress=&data_display='
           '&per_page=50')
    countItems = get_geoData(url, url1, url2, countItems)
    countCoord = countCoordFunction(countCoord)
    result = ({"Название набора данных": 'Адреса установки стационарных фоторадарных комплексов, фиксирующих нарушения ПДД на дорогах г.Санкт-Петербурга (Версия №10 от 26.06.2023)',
               "Ссылка на набор данных": str(url),
               "Количество записей в наборе": str(countItems),
    ...
    countCoord = [0, 0, 0]
    url = (
       'https://data.gov.spb.ru/api/public/version/5227/structure_version/387/?search_all=%D1%80%D0%B5%D1%81%D1%83%D1%80%D1%81%D0%BE%D1%81&page=1&name=&address_yur=&address_fact=&phone=&email=&site=&data_display=&per_page=5'
    )
    ...
    countItems = 0
    countCoord = [0, 0, 0]
    url = ('https://data.gov.spb.ru/api/public/version/5260/structure_version/618/?search_all=%D0%BA%D0%B0%D1%82%D0%BA%D0%B8&page=1&number=&name=&scope=&type=&address=&dop=&metro=&district=&phone=&site=&status=&time=&time_day=&location=&price=&reach=&others=&lighting=&data_display=&per_page=50'
           )
    url1 = ('https://data.gov.spb.ru/api/public/version/5260/structure_version/618/?search_all=%D0%BA%D0%B0%D1%82%D0%BA%D0%B8&page=')
    url2 = (
        '&number=&name=&scope=&type=&address=&dop=&metro=&district=&phone=&site=&status=&time=&time_day=&location=&price=&reach=&others=&lighting=&data_display=&per_page=50'
    )
    countItems = get_geoData(url, url1, url2, countItems)
    countCoord = countCoordFunction(countCoord)
    result = ({
        "Название набора данных": 'Катки и лыжные трассы Санкт-Петербурга (Версия №25 от 03.04.2023)',
        "Ссылка на набор данных": str(url),
        "Количество записей в наборе": str(countItems),
        "Название столбца с координатами": nameCoordJason,
        "Колич коорд на терр РФ": str(countCoord[0]),
    ...
    dfFinish = pd.DataFrame(finishTable)   # запись результатов в excel
